...
export function ActiveCallsTab({ calls, isLoading, resolveCall }: any) {
  ...
  if (!calls || calls.length === 0) {
    return (
      <div className="text-center py-16 text-muted-foreground">
        ...
        <p className="text-sm opacity-50 mt-2">{t('staff_can_rest', 'Персонал може відпочивати')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      {calls.map((call: any) => (
        <div key={call.id} className="flex flex-col p-4 bg-card border-2 border-border/50 rounded-2xl shadow-sm gap-4">
